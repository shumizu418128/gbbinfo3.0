...
# MARK: 世界地図（非同期版）
async def world_map_view_async(request, year: int):
    """
    世界地図ビュー関数（非同期版）。
    """
    # 言語はクッキーから取得（Sanicでは）
    language = request.cookies.get("language", "ja")
    ...
    if os.path.exists(map_save_path):
        return await render(f"{year}/world_map/{language}.html")
    ...
    country_coordinates_data = await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: supabase_service.get_data(
            table="Country",
            columns=["iso_code", "latitude", "longitude", "names"],
    ...
    await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: beatboxer_map.save(map_save_path)
    )
